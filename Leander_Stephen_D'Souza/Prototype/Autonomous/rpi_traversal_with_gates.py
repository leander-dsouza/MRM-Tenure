...
gate_no=input("ENTER THE NUMBER OF GATES:-")
latarr=[]
lonarr=[]
for coun in range(gate_no):
	latarr.append(float(input("ENTER LATITUDE:")))
	lonarr.append(float(input("ENTER LONGITITUDE:")))
cood=0
...
try:
    for new_data in gps_socket:
        ...
        if new_data:
            ...
            dist = haversine(lat1, lon1, latarr[cood], lonarr[cood])
            if dist < 7:
                print("GATE", cood + 1, "REACHED")
                # execute ball detection
                #os.system(r"/home/pi/rpi_atmega_camera.py")
                subprocess.Popen(['python',r'/home/pi/rpi_atmega_camera.py']).wait()
                cood += 1

                if cood == gate_no:
                    brutestop()
                    quit()
            a = np.round(bearing(lat1, lon1, latarr[cood], lonarr[cood]))
